...
<div class="max-w-3xl mx-auto px-4 sm:px-6 pt-10 pb-12">
    <div class="rounded-3xl p-8 sm:p-10 shadow-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 transition-colors">
        <div class="space-y-3 mb-8">
            ...
            <h1 class="text-2xl sm:text-4xl font-black text-slate-900 dark:text-white">Identificação do Regime Tributário</h1>
        ...
        <form action="{{ route('tax-regime.submit') }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                <label for="regime_tributario" class="block text-sm font-semibold text-slate-900 dark:text-slate-200">3. Qual é o regime tributário atual da empresa?</label>
                <select name="regime_tributario" id="regime_tributario" required
                    class="block w-full px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm focus:border-red-500 focus:ring-red-500 transition">
                    <option value="">Selecione...</option>
                    @foreach (['Simples Nacional','Lucro Presumido','Lucro Real','MEI'] as $opt)
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-6 mt-3">
                    <label class="inline-flex items-center gap-2 cursor-pointer text-slate-800 dark:text-slate-200 text-sm">
                        <input type="radio" name="lc_214_2025_compliant" value="1" required class="text-red-600 focus:ring-red-500 border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700">
                        <span>Sim</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer text-slate-800 dark:text-slate-200 text-sm">
                        <input type="radio" name="lc_214_2025_compliant" value="0" required class="text-red-600 focus:ring-red-500 border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700">
                        <span>Não</span>
                    </label>
                </div>
            </div>
            ...
            <div class="flex justify-end pt-4">
                <button type="submit" class="inline-flex items-center gap-2 px-6 py-3.5 rounded-2xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-bold shadow-md hover:shadow-lg transition">
