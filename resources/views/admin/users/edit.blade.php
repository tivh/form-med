...
<div class="max-w-4xl mx-auto px-4 pt-12 pb-10">
    <div class="rounded-3xl bg-gradient-to-r from-red-500 to-rose-500 text-white shadow-xl overflow-hidden mb-8">
        <div class="p-8 md:p-10 space-y-3">
            ...
            <p class="text-white/80">Atualize os dados e as visões de acesso de {{ $user->name }}.</p>
        </div>
    </div>
                ...
                <div class="space-y-4 pt-2">
                    ...
                    <div class="p-4 rounded-xl bg-slate-50 border border-slate-200">
                        <label class="flex items-start gap-3 cursor-pointer">
                            <input type="checkbox" id="is_super_admin" name="is_super_admin" value="1" 
                                {{ old('is_super_admin', $user->isSuperAdmin()) ? 'checked' : '' }}
                    ...
                    <div id="areas-selection-container" class="space-y-4">
                        <div>
                            ...
                            @php 
                                $userAreaSlugs = $user->areas->pluck('slug')->toArray();
                                if (empty($userAreaSlugs) && !empty($user->form_scope)) {
                                    $userAreaSlugs = [$user->form_scope];
                                }
                                $selectedAreas = old('areas', $userAreaSlugs);
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function toggleVisibility() {
            if (!superAdminCheckbox || !areasContainer) return;
