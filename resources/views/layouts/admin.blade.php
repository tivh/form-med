...
<nav class="bg-slate-900 border-b border-slate-700 sticky top-[69px] z-10">
    <div class="max-w-6xl mx-auto px-4 flex items-center justify-between">
        ...
        <div class="flex items-center">
            ...
            {{-- Compliance --}}
            @if(auth()->user()->canAccess('form-med'))
            ...
            {{-- Financeiro --}}
            @if(auth()->user()->canAccess('regime-tributario'))
                <a href="{{ route('admin.tax-regime.index') }}"
                   class="inline-flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition
                   {{ request()->routeIs('admin.tax-regime.*') ? 'border-red-400 text-white' : 'border-transparent text-slate-400 hover:text-white hover:border-slate-500' }}">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                              d="M3 7.5h18M7 3v4.5M17 3v4.5M5.25 21h13.5A2.25 2.25 0 0021 18.75V7.5H3v11.25A2.25 2.25 0 005.25 21z"/>
                    </svg>
                    Regime Tributário
                </a>
            @endif
        ...
        <form action="{{ route('logout') }}" method="POST" class="flex items-center py-2">
            @csrf
            <button type="submit"
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-700 transition">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
